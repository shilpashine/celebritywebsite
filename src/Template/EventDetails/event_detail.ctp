<?php 
$baseurl='http://localhost/git_project/WebIdeasole/celebritycalling/'
...
?><main id="main" class="site-main">
    ...
    <div id="jssor_1" style="position:relative;margin:0 auto;top:0px;left:0px;width:980px;height:910px;overflow:hidden;visibility:hidden;">
        <!-- Loading Screen -->
        <div data-u="slides" style="cursor:default;position:relative;top:0px;left:0px;width:980px;height:910px;overflow:hidden;">
            <?php $data_all_event=json_decode($data_all_event);?>
            <?php foreach($data_all_event->event_photos as $val1){ ?>
            <div>
        ...
        <!-- Thumbnail Navigator -->
        <div data-u="thumbnavigator" class="jssort111" style="position:absolute;left:0px;bottom:0px;width:980px;height:150px;cursor:default;" data-autocenter="1" data-scale-bottom="0.75">
            <div data-u="slides">
    ...
    <!-- #endregion Jssor Slider End -->
                                                  ...
                                                  <?php  if(!empty($data_all_event)){ //pr($data_all_event);exit;?>
							<div class="campaign-box">
								<a href="#" class="category"><?php echo $data_all_event->event_categories[0]->category->category_name;?></a>
  ...
  aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:<?php if(!empty( $data_all_event->event_amount)&& !empty( $data_all_event->target_amount) ) {  $new_amount=($amount11/$data_all_event->event_amount)*100 ; echo round($new_amount);}?>%">
									...
									<div class="process-info">
										...
										<div class="process-time"><span><?php if(!empty($data_all_event->approx_start_date)){
                                                                                                                                                ...
                                                                                                                                                $upcomming_date=date_create(date("Y-m-d",strtotime($data_all_event->approx_start_date)));
...
<!--								<div class="button">
									<form action="#" class="campaign-price quantity">
										<input type="number" value="1" min="0" name="s" placeholder="" />
                                                                                <a href="../../../../../../../../../C:/Users/Shaggy/Downloads/celebrity-fontend-cur/celebrity-fontend (2)/event_details.html"></a>
										<a class="btn-primary" href="thank_you.html" target="_blank">Book Tickets</a>
									</form>
						...
						<div class="col-lg-8">
							<div class="campaign-tabs">
								<ul class="tabs-controls">
									<li class="active" data-tab="campaign"><a href="#">Events Story</a></li>
...
<!--                                                                                                                        <a href="#" class="comment-reply" onclick="reply(1)">Reply</a>-->
						...
						<div class="col-lg-4">
							<div class="support support-campaign">
								<h3 class="support-campaign-title">Back this Events</h3>
                                                                <?php foreach($data_all_event->event_ticket_details as $val){ ?>
								<div class="plan">
									<a href="javascript:void(0)">
										<h4>Pledge <i class="fa fa-inr" aria-hidden="true"></i><?php echo $val->ticket_name; ?> - <i class="fa fa-inr" aria-hidden="true"></i> <?php echo $val->ticket_price; ?></h4>
										<div class="plan-desc"><p><?php echo $val->ticket_desc; ?></p></div>
<!--										<div class="plan-date">May, 2017</div>
										<div class="plan-author">Estimated Delivery</div>-->
										<div class="backer"><?php echo $val->ticket_qty; ?> backer</div>
                                                                                   <div class="process-time"><span><?php echo $val->ticket_avl;?></span>available</div>
									</a>
                                                                    <div class="button book-tick">
                                                                        <form action="" method="post" class="campaign-price quantity">
                                                                              <input type="hidden" value="<?php echo $val->ticket_avl; ?>" name="ticket_avl1" id="ticket_avl<?php echo $val->id; ?>"/>
                                                                            <input type="hidden" value="<?php echo $val->id; ?>" name="ticket_id" />
                                                                            <input type="number" value="1" min="1" name="qty" id="less_ticket<?php echo $val->id; ?>" placeholder="" onkeyup="check_qty(<?php echo $val->id; ?>)" />
                                                                            <div id="error_val<?php echo $val->id; ?>"></div>
                                                           <?php                     if(!empty($AuthUser)) {   ?>
                                                                            <input type="submit" name="submit" id="book_id<?php echo $val->id; ?>" class="btn-primary"  value="Book Tickets"/>
								
<?php } else {
    echo $this->Html->link('Login For Book', array('controller' => 'users', 'action' => 'login'),array('class'=>'btn-primary')); 
...
<!--								<div class="plan">
									<a href="javascript:void(0)">
										<h4>Pledge <i class="fa fa-inr" aria-hidden="true"></i> 200 - <i class="fa fa-inr" aria-hidden="true"></i> 300</h4>
									...
									<form action="#" class="campaign-price quantity">
										<input type="number" value="1" min="0" name="s" placeholder="" />
										<a class="btn-primary book-butt" href="thank_you.html" target="_blank">Book Tickets</a>
									</form>
									 <a href="#" class="btn-secondary"><i class="fa fa-heart" aria-hidden="true"></i>Remind me</a> 
								</div>
								</div>-->
    ...
    if(less_ticket==0){
         $("#error_val"+el).html('Enter Proper Qty');
    }
