							...
							<div class="col-lg-6" id="show_data" name="show_data">
								<div class="card-box">
									<h4 class="m-t-0 header-title"><b>Franchise Add</b></h4>
									<p class="text-muted font-13 m-b-30">
	                                </p>
									<form action="add_franchise_data" method="post" name="theAddForm" id="theAddForm"data-parsley-validate enctype="multipart/form-data" >
									    <div class="form-group">
											<label for="fname">Franchise First Name*</label>
										...
										<div class="form-group">
											<label for="lname">Franchise Last Name*</label>
										...
										<div class="form-group">
											<label for="phone_number">Franchise Phone Number*</label>
										...
										<div class="form-group">
											<label for="pswd_token">Franchise Password*</label>
                                    ...
                                    <table id="datatable-buttons" class="table table-striped table-bordered">
                                        ...
                                        <tbody>
										       <?php 
												...
												foreach($franchise_data as $franchise_data_val){
												?>
                                            <tr>
                                                <td><?php echo $franchise_data_val->fname." ".$franchise_data_val->lname;?></td>
                                                <td><?php echo $franchise_data_val->username;?></td>
                                                <td><?php echo $franchise_data_val->created;?></td>
                                                <td><?php echo $franchise_data_val->modified;?></td>
                                                <td>
												<a title="View" onclick="view_data(<?php echo $franchise_data_val->id;?>)" href="#"><i class="fa fa-eye"></i></a>
												<a title="Edit" onclick="edit_data(<?php echo $franchise_data_val->id;?>)" href="#"><i class="fa fa-edit"></i></a>
												<a title="Delete" href="<?php echo $this->Url->build('/admin/users/delete_data/', true).$franchise_data_val->id ; ?>"><i class="fa fa-trash-o"></i></a>
												<a title="Status" href="<?php echo $this->Url->build('/admin/users/change_status/', true).$franchise_data_val->id; ?>"> <?php if($franchise_data_val->status==1){echo $this->Html->image("/img/test-pass-icon.png"); } else{ echo $this->Html->image("/img/warning.png");}?></a>
												</td>
                                            </tr>
												<?php }}?>
                                        </tbody>
...
function view_data(user_id){
        ...
        type: "POST",
        evalScripts: true,
        url: '<?php echo $this->Url->build('/admin/users/view_franchise_data', true);  ?>',
        data: ({id:user_id}),
...
function edit_data(user_id){
		 $.ajax({
				type: "POST",
				evalScripts: true,
				url: '<?php echo $this->Url->build('/admin/users/ajax_franchise_edit_data', true);  ?>',
				data: ({id:user_id}),
				success: function (data){
				$('#show_data').html(data);
				}
