...
class PoliciesController extends AppController {
	    ...
	    if(!empty($user)){
		  if($user['user_type']!=2){
			return $this->redirect($this->Auth->logout());
			 	...
			 	$this->Auth->allow(['login','logout','editProfile','listPolicy','viewPolicy','fetchPolicy','editPolicy']);
	...
	public function listPolicy(){
        ...
        $datas=$this->Policies->find('all', array(
         'recursive' => -1,
		 'contain' => array(
	    ),
	    'conditions'=>array(
				'Policies.is_deleted' =>0,
				'Policies.user_id' =>$userdata['id'],
				'Policies.created' =>date("Y-m-d")
                ),
                'order' => array(
                'Policies.id' => 'DESC'
                )
        ));
		$policy_data = $datas->toArray();
		$this->set('policy_data',json_encode($policy_data));
		if($this->request->is("post") or $this->request->is("put")){
		//pr($this->request->data);
        $policy = $this->Policies->newEntity(); 
		$policy->user_id=$userdata['id'];
		$policy->modifier_id=$userdata['id'];
		$policy->created_by=$userdata['fname']." ".$userdata['lname'];
		$policy->modified_by=$userdata['fname']." ".$userdata['lname'];
		$policy->creator_type="Sales Operator";
		$policy->modifier_type="Sales Operator";
        $policy_date=$this->request->data['policy_date'];
		...
		//$policy->user_id=$userdata['id'];
		//$policy->user_type="OperatorAdmin";
		$policy->company_name=$this->request->data['company_name'];
		$policy->fname=$this->request->data['fname'];
		$policy->lname=$this->request->data['lname'];
		$policy->address1=$this->request->data['address1'];
		$policy->address2=$this->request->data['address2'];
		$policy->city=$this->request->data['city'];
		$policy->pin=$this->request->data['pin'];
		$policy->gst=$this->request->data['gst'];
		$policy->sum_assured=$this->request->data['sum_assured'];
		$policy->amount=$this->request->data['amount'];
		$policy->gross_premium=$this->request->data['gross_premium'];
		$policy->created=date("Y-m-d H:i:s");
		$policy->modified=date("Y-m-d H:i:s");
         if ($this->Policies->save($policy)) {
		  $this->Flash->set('The data has been successfully added.');
		  $this->redirect(array("controller"=>"policies","action"=>"list_policy"));
         }
		 else{
	...
	public function viewPolicy(){
        ...
        if(!empty($userdata)){		
		 ...
		 //echo $lead_id;exit;
		$policy_view_datas=$this->Policies->find('all', array(
         'recursive' => -1,
		 'contain' => array(
	    ),
	    'conditions'=>array(
		...
		$policy_view_data = $policy_view_datas->first();
		//pr($lead_remark_data);exit;
		if(!empty($policy_view_data)){
		$this->set('policy_view_data',json_encode($policy_view_data));
		}
		}
		else
		{
	...
	public function fetchPolicy(){
        ...
        if(!empty($userdata)){		
		 $policy_id=$this->request->data['policy_id'];
		 //echo $lead_id;exit;
		$policy_fetch_datas=$this->Policies->find('all', array(
         'recursive' => -1,
		 'contain' => array(
	    ),
	    'conditions'=>array(
		...
		$policy_fetch_data = $policy_fetch_datas->first();
		//pr($lead_fetch_data);exit;
		if(!empty($policy_fetch_data)){
		$this->set('policy_fetch_data',json_encode($policy_fetch_data));
		}
		}
		else
		{
	...
	public function editPolicy() {
		   ...
		   if($this->request->is("post") or $this->request->is("put")){
			...
			$policy_id=$this->request->data["edit_policy_id"];
			$edit_policy=$this->Policies->get($policy_id);
			$edit_policy->modifier_id=$userdata['id'];
			$edit_policy->modified_by=$userdata['fname']." ".$userdata['lname'];
			$edit_policy->modifier_type="Sales Operator";
			$edit_policy->policy_date=$this->request->data['edit_policy_date'];
			$edit_policy->company_name=$this->request->data['edit_company_name'];
			$edit_policy->fname=$this->request->data['edit_fname'];
			$edit_policy->lname=$this->request->data['edit_lname'];
			$edit_policy->address1=$this->request->data['edit_address1'];
			$edit_policy->address2=$this->request->data['edit_address2'];
			$edit_policy->city=$this->request->data['edit_city'];
			$edit_policy->pin=$this->request->data['edit_pin'];
			$edit_policy->gst=$this->request->data['edit_gst'];
			$edit_policy->sum_assured=$this->request->data['edit_sum_assured'];
			$edit_policy->amount=$this->request->data['edit_amount'];
			$edit_policy->gross_premium=$this->request->data['edit_gross_premium'];			
			$edit_policy->modified=date("Y-m-d H:i:s");
			if($this->Policies->save($edit_policy))
				{
				    $this->Flash->set('The data has been successfully updated.');
					$this->redirect(array("action"=>"listPolicy"));	
				}
